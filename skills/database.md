...
Re-export everything from `db/schema/index.ts`. This is the one exception alongside the feature root `index.ts` allowed by `imports-and-exports.md` — a schema barrel, because Drizzle relations require all tables in one graph.
...
These are DTO types — server/database contracts. Per `typescript.md`, never use them directly as component props; map them to application types at the feature boundary. See "Mappers" in `api-and-data-fetching.md`.
...
- Server Actions (Next.js monolith — see `nextjs-server-actions.md`)
- Route Handlers / `app/api`
- Hono route handlers (standalone API package — see `monorepo.md`)
