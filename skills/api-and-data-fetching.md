...
**Scope**: this document applies to Vite + React SPA projects, and to any client (including a Next.js app inside a Turborepo monorepo) that calls a standalone backend service such as a Hono API — see `monorepo.md`. For a Next.js monolith talking directly to its own database, Server Actions are the primary data layer instead — see `nextjs-server-actions.md`.
...
Never use raw response types as component props — map them to the application type first, per `typescript.md`.
