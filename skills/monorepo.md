...
Use Bun as the runtime and package manager inside a monorepo (`bun install`, `bun run`, workspaces via `bunfig.toml` / `package.json#workspaces`).
...
├── packages/
│   ├── database/      # Drizzle schema + client, see database.md
...
Each app in `apps/` is a full application and internally follows `folder-structure.md` (its own `src/features`, `src/shared`, etc.) exactly as a standalone project would.
...
The web app consumes this API the standard client-side way — Axios + TanStack Query, per `api-and-data-fetching.md`. Server Actions (`nextjs-server-actions.md`) are for a Next.js monolith talking to its own database directly, not for calling a separate Hono service.
...
Drizzle schema, client, and migrations. See `database.md`. Only this package touches the database directly.
...
- Use Bun for install/run/workspaces.
...
- Each app still follows `folder-structure.md` internally.
