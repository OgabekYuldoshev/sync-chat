...
Server state is covered separately in `api-and-data-fetching.md`. This document only covers client-only state.
...
Server state belongs to TanStack Query. See `api-and-data-fetching.md`.
...
Store files must end with `-store`, per `naming-conventions.md`.
