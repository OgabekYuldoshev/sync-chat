...
Exception: Next.js App Router convention files (`page.tsx`, `layout.tsx`, `loading.tsx`, `error.tsx`, `not-found.tsx`, `route.ts`) require a default export by framework contract. See `nextjs.md`. Keep these files thin — re-export a named implementation from the feature.
...
Import order is enforced automatically by Biome's `organizeImports` config — see `tooling.md`. Do not hand-order imports; run `biome check --write` and let it sort them.
